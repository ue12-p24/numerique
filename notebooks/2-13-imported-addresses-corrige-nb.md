...
```{code-cell} ipython3
%timeit localize_one(18, 'rue', 'tourelles')
```
...
* et donc dans notre cas, puisque `data` est un paramètre de type fichier, alors que `columns` est un paramètre usuel, on fera
  ```python
  response = requests.post(url, files={'data': file-obj}, data={'columns': ['col1', ...]})
  ```
...
def localize_many(filename, col_number, col_type, col_name, col_city):
    ...
    Parameters:
      filename:
        the name of the inpuxt csv file
      col_number:
      col_type:
      col_name:
      col_city:
...
def mass_post(filename, col_number, col_type, col_name, col_city):
    ...
    with open(filename, encoding="utf8") as feed:
...
# your code
sum((addresses_small.result_type != 'housenumber') | (addresses_small.result_city != 'Paris'))
```
...
```{code-cell} ipython3
sum(addresses.result_type != 'housenumber')
```

```{code-cell} ipython3
sum(addresses.result_city != 'Paris')
```

```{code-cell} ipython3
sum( (addresses.result_type != 'housenumber') | (addresses.result_city != 'Paris'))
```
...
def map_addresses(geoloc):
    ...
    # avoid warning
    if 'human' not in geoloc.columns:
...
def add_quartiers(gdf):
    # this is required in recent versions of pandas
